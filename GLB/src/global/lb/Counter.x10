...
def verboseStats(h:Int, sumCounters:Counter) {
	...
	public def stats(allCounters: Rail[Counter], time:Long, verbose:Boolean) {
		// assert here.id == 0;
		val P = Place.MAX_PLACES;
	...
	static val NODES = 6;
	static val LIFE = 7;
	def computeTime(i:Int, P:Int, allCounters: Rail[Counter]) = 
		computeTime(i, P, allCounters, 1, "");
	def computeTime(i:Int, P:Int, allCounters: Rail[Counter], divider:Int, unit:String):Stat {
