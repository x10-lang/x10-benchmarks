...
n_{u,v}(t) = \left\{ \begin{array}{rr} 
  n_{u,t}\times{}n_{t,v} & d_{u,t}+d_{t,v}=d_{u,v} \\
  0 & otherwise \\
\end{array}\right.
\label{eq:bcoft}
\end{equation}
...
%
For example, BC($d$) and BC($c$) in $G$ (from Figure~\ref{fig:sample}) are
both $1/2$ as there are exactly two shortest paths between $b$ and $a$, and 
$c$ and $d$ are on these paths.
%
...
\end{equation}
%
\todoanju{Check this equation again; I might have confused lemmas $3$ and $5$
from Brandes' algorithm.}
%
...
\end{align*}
%
This change in the $BC_b$ denotes that $b$ is on \textit{all} shortest paths
from $a$ to $c$ and $a$ to $d$.
%
...
\end{align*}
%
At the end of the first pass, we have expanded vertex $b$ and discovered 
vertices $c$ and $d$; $nsp$ now indicates that we have found one shortest path
$(a,b)$.
%
\begin{align*}
...
\end{align*}
%
At the end of the second pass, we have explored $c$ and $d$; at this point
