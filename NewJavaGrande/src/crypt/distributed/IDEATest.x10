...
public class IDEATest {
	...
	var plain1: DistArray[byte](1);       // Buffer for plaintext data.
	var crypt1: DistArray[byte];       // Buffer for encrypted data.
	var plain2: DistArray[byte];       // Buffer for decrypted data.

	var userkey: Array[int];         // Key for encryption/decryption.
	...
	def buildTestData(): void = {
		...
		userkey = new Array[int](8);  // User key has 8 16-bit shorts.
		...
		plain1 = DistArray.make[byte](D, ([i]: Point(1)): byte => { return i as byte; });
	...
	private def calcEncryptKey(): void {
		...
		val Z: Array[int] = new Array[int](52);
		for ([i]: Point in 0..7) {
		...
		ZR = DistArray.make[int](Dist.makeConstant(0..(Z.size-1), here), ([i]: Point): int => { return Z(i); });
	...
	private def calcDecryptKey(): void {
		...
		val DK: Array[int] = new Array[int](52);
		val Z: DistArray[int] = ZR;
		...
		DKR = DistArray.make[int](Dist.makeConstant(0..(DK.size-1), here), ([i]: Point): int => { return DK(i); });
	 ...
	 * fit in 16 bits.
	 */
	private def cipher_idea( text1: DistArray[byte], text2: DistArray[byte], key: DistArray[int]): void {
		// WARNING: This assumes that the size of each block is a multiple of 8!
		// Console.OUT.println( "region = " + text1.region() ) ;
		// Console.OUT.println( "size = " + text1.region().size() ) ;
		if ( text1 != null && text1.region().size() % (8 * Place.MAX_PLACES) != 0) {
			throw new RuntimeException("Invalid number of places for the distribution size");
		}
		for ([i]: Point in text1.dist | here) if (i % 8 == 0) {
