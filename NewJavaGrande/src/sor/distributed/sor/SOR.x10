...
public class SOR {
    ...
    static val gTotal: Cell[Double] = new Cell[Double](0.0 as double);

    public static def read(G: DistArray[Double], i: int, j: Int) {
		return at (G.dist(i, j)) { G(i, j) };
    }

	public static def SORrun(omega: Double, G: DistArray[Double], numIter: Int) {
		...
		for ([p]: Point in 0..(numIter-1)) 
		    for ([o]: Point in 0..1) 
			finish for ([ii]: Point in 0..(((Mm1-1-(1+o))/2))) async {
			val i: int = 2 * ii + 1 + o;
			finish at (G.dist(i, 1)) async {
		...
		gTotal.value = G.reduce((a:double, b:double)=>a+b,0.0 as double);
	}
}
