...
public class JGFSORBench extends SOR implements JGFSection2 {
	...
	val dev = Math.abs(gTotal.value - refval(size));
	if (dev > 1.0e-12) {
	    Console.OUT.println("Validation failed");
	    Console.OUT.println("gTotal = " + gTotal.value + "  " + dev + "  " + size);
    ...
    private static def write(t: DistArray[Double],  i: Int, j: Int, v: Double) {
