...
public class SSCA2TaskFrame extends TaskFrame[SSCA2TaskItem, SSCA2Result]{
	...
	// 	this.interval = interval;
	// 	this.ssca2 = new SSCA2(this.graph, this.verbose);
	// 	if(permute > 0){
	// 		ssca2.permuteVertices();
	// 	}
	// }
	...
	public def runAtMostNTask(var n:Long):Boolean {
		...
		while((tb.size() > 0) && (tasksRun < n)){
			...
			ssca2.bfsShortestPaths(vStartIdx, eStartIdx+1n); // NOTE: the +1n in the end, that is [start, end) is the region that SSCA2 works on when calling bfsShortestPaths
			tasksRun++;
		}
		return (tasksRun==n);
	}
	...
	public def getResult():SSCA2Result {
		...
		return new SSCA2Result(this.ssca2.betweennessMap);
	}
	...
	public def printResult():void{
		this.ssca2.printBetweennessMap();
	}
