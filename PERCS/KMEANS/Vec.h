...
template <class T, x10_int SZ> struct NativeVec : public NativeVec_rtt<T> {
    ...
    {
        return x10::lang::String::__plus(x10::lang::String::Lit("struct Vec: size="),
                                         x10aux::to_string(SZ));
    }
...
template<class T> x10aux::RuntimeType NativeVec_rtt<T>::rtt;

template<class T> void NativeVec_rtt<T>::_initRTT() {
    const x10aux::RuntimeType *canonical = x10aux::getRTT<NativeVec_rtt<void> >();
