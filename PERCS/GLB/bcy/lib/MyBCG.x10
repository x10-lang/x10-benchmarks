...
public final class MyBCG {
    public static def main(args:Rail[String]):void {
        ...
        val init = ()=>{ return new MyQueue(Rmat(seed, n, a, b, c, d), permute, yfStr); };
        val glb = new GLB[MyQueue](init, g, w, l, z, m, false);
        ...
        if(verbose > 0) {
            PlaceGroup.WORLD.broadcastFlat(()=>{
                Console.OUT.println("[" + here.id + "]"
                        + " Time = " + (glb.taskQueue().bc_).accTime
                        + " Yield Time = " + ((glb.taskQueue().bc_).accYTime/1e9)
                        + " Count = " + (glb.taskQueue().bc_).count);
            });
        }

        if(verbose > 2) (glb.taskQueue().bc_).printBetweennessMap(6n);

        val bc = glb.taskQueue().bc_;
