...
public class Output {
   ...
   public def this(score_: Long,                        // the value calculated for the best aligned match
		   ...
		   shortBuilder: ArrayBuilder[Byte], // the short aligned string, reversed
		   longBuilder: ArrayBuilder[Byte]   // the long aligned string, reversed
    ...
    * 
    * Programming note: If we used a ArrayBuilder, if our target hardware could exploit
    ...
    */
   private static def cleanup(builderIn: ArrayBuilder[Byte]) {
      val source = builderIn.result();
      val builderOut = new StringBuilder();
      for(var i: Int = source.size-1; i>=0; i--) builderOut.add(source(i) as Char);
